...
from src.workloads.cloudlab.loadgen.loadgenerator import project2, csrf, randomwords
# import metrics
import logparser
import socket
import datetime
...
# GET_PROJECT_LIST_TIMEOUT = 3.853
# GET_PROJECT_LIST_TIMEOUT = 10

LOGOUT_TIMEOUT= 1.455
# LOGOUT_TIMEOUT= 100
...
DOWNLOAD_PROJECT_HOME_TIMEOUT = 100
# GET_PROJECT_LIST_TIMEOUT = 0.1
# LOGOUT_TIMEOUT= 0.1
# LOGIN_TIMEOUT= 0.1
# SPELL_CHECK_TIMEOUT= 0.1
# HISTORY_TIMEOUT= 0.1
# TAG_TIMEOUT = 0.1
# DOCUMENT_DIFF_TIMEOUT = 0.1
# GET_SETTINGS_TIMEOUT = 0.1
# UPDATE_SETTINGS_TIMEOUT = 0.1
# UPLOAD_FILE_TIMEOUT = 0.1
# COMPILE_TIMEOUT = 0.1
# GET_COMPILE_PDF_TIMEOUT = 0.1
# GET_CONTACTS_TIMEOUT = 0.1
# SHARE_PROJECT_TIMEOUT = 0.1
...
def save_raw_stats(filename):
    print("#success: %s, fail: %s, error: %s #" % (nr_success, nr_failed, nr_error))
    print("#pos triggered: %s, pos_registered: %s #" % (project2.pos_triggered, project2.pos_registered))
...
def login(l):
    ...
    resp = l.client.get("/login", timeout = LOGIN_TIMEOUT, name='get_login_page')
    ...
    logging.info("[Phoenix] {} get_login_page {} {}".format(str(datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")), str(success), str(response_time)))
    resp = l.client.post("/login", data, catch_response=True, timeout=LOGIN_TIMEOUT,  name="login")
    success = resp.status_code == 200
    response_time = resp.elapsed.total_seconds() * 1000
...
@task
def download_project_home(l):
    r = l.client.get("/project", timeout = DOWNLOAD_PROJECT_HOME_TIMEOUT, name='get_download_home_project_lists')
    if r.status_code == 200:
        success = True
        response_time = r.elapsed.total_seconds() * 1000
        logging.info("[Phoenix] {} get_download_home_project_lists {} {}".format(str(datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")), str(success), str(response_time)))
    else:
        success = False
        response_time = -1
        logging.info("[Phoenix] {} get_download_home_project_lists {} {}".format(str(datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")), str(success), str(response_time)))
    ...
    if len(projects) > 0:
        ...
        if random_number <= 0.05:
            ...
            r = l.client.get("/project/%s/download/zip"  % project_id, timeout = DOWNLOAD_PROJECT_HOME_TIMEOUT, name='download_home_project')    
            if r.status_code == 200:
                success = True
                response_time = r.elapsed.total_seconds() * 1000
                logging.info("[Phoenix] {} download_home_project {} {}".format(str(datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")), str(success), str(response_time)))
            else:
                success = False
                response_time = -1
                logging.info("[Phoenix] {} download_home_project {} {}".format(str(datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")), str(success), str(response_time)))
...
def settings(l):
    ...
    r = l.client.get("/user/settings", timeout = GET_SETTINGS_TIMEOUT, name='get_settings')
    ...
    # timeout = UPDATE_SETTINGS_TIMEOUT,
...
def logout(l):
    ...
    resp = l.client.get("/logout", timeout = LOGOUT_TIMEOUT, name='logout')
    success = resp.status_code == 200
    response_time = resp.elapsed.total_seconds() * 1000
    # global RESULT_WRITER
...
class ProjectOverview(TaskSet):
    tasks = { project2.Page: 10, create_tag: 10, settings: 5, logout: 20, download_project_home: 200}
    # tasks = { project2.Page: 30, create_tag: 10, settings: 5, logout: 20, download_project_home: 100}
    # tasks = { project.Page: 0, create_tag: 0, settings: 0, logout: 0}
    if len(PROJECT_OVERVIEW_TASKS):
        t = json.loads(PROJECT_OVERVIEW_TASKS)
        if 'project.Page' in t: tasks[project2.Page] = t['project.Page']
