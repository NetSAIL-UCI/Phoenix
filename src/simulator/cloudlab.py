...
def move_to_front(lst):
    # Separate items with "web" or "front-end" and other items
    front_items = [(i, item) for (i, item) in lst if "web" in item or "frontend" in item]
    other_items = [(i, item) for (i, item) in lst if "web" not in item and "frontend" not in item]
    
    # Combine them with front_items at the beginning
    return other_items + front_items
...
def run_planner(remaining_capacity, graphs, pname="phoenix"):
    ...
    if "phoenixfair" == pname:
        ...
        planner.nodes_to_activate = move_to_front(planner.nodes_to_activate)
        # planner.nodes_to_activate = [(1, 'web'), (4, 'web'), (3, 'web'), (0, 'frontend'), (2, 'frontend'), (4, 'tags'), (1, 'tags'), (3, 'tags'), (3, 'notifications'), (0, 'rate'), (2, 'reservation'),  (3, 'real-time'), (0, 'geo'), (0, 'search'), (3, 'document-updater'), (4, 'real-time'), (4, 'document-updater'), (1, 'document-updater'), (1, 'notifications'), (4, 'notifications'),  (4, 'clsi'), (3, 'spelling'), (2, 'profile'), (3, 'contacts'), (1, 'clsi')]
    elif "default" == pname:
...
def run_cloudlab():
    ...
    work_dir = "asplos_25/"
    fname = work_dir + "cloudlab_results_ae.csv"
    with open(fname, "w") as out:
        hdr = "num_servers,deployment_id,failure_level"
        sys_names = ["priority", "lpunifiedfair", "lpunified", "phoenixfair","phoenixcost","fairDG", "default","priorityminus"]
        # sys_names = ["phoenixcost", "lpunifiedfair", "lpunified", "phoenixfair","fairDG", "default","priority", "priorityminus"]
    ...
    for seed in [1, 2, 3]:
        ...
        for to_del in [7]:
            ...
            to_del = ['node-19', 'node-18', 'node-23', 'node-24', 'node-21', 'node-20', 'node-22']
            ...
            with open(fname, "a") as out:
                ...
                for pname in sys_names:
                    ...
                    final_pods, proposed_pod_to_node, graphs, idx_to_ns = run_system(new_cap, copy.deepcopy(destroyed_state), pname, list(namespaces), pod_resources)
                    # if pname == "lpunified":
                    #     planner = LPUnified(graphs, state, fairness=False)
                    #     final_pods = planner.final_pods
