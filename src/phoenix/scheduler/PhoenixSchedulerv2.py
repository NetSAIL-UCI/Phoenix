...
class PhoenixSchedulerv2:
    ...
    def is_deletion_feasible(self, candidates, ms, rank_ms):
        # candidates = [-1, -2, -3, -4]
        candiate_found = False
        best_candidate = None
        best_pods_to_remove = None
        for nodeid in candidates:
            # check if there are scheduled pods of rank > rank_ms and whether turning them off will free up space or not.
            # empty_space = 
            ...
            for pod, pod_size in self.bins[nodeid]:
                if pod not in self.pod_ranks:
                    ...
                    if target <= 0:
                        ...
                        break
                elif self.pod_ranks[pod] > rank_ms:
                    target -= pod_size
                    pods_to_remove.append(pod)
                    if target <= 0:
                        candiate_found = True
    ...
    def delete(self, ms, rank_ms):
        ...
        #         continue
        # candidate = self.find_deletion_candidates(ms, rank_ms)
        while self.scheduled[-1][0] > rank_ms:
            ...
            if self.Eopt[-1][0] >= self.container_resources[ms]:
                ...
                break
            # Delete all replicas
            candidates_for_targetted_deletion = []
            for pod in pods:            
                ...
                start_in4 = time()
                self.Eopt.add((val, index))
                candidates_for_targetted_deletion.append(index)
            ...
            if self.Eopt[-1][0] >= self.container_resources[ms]:
                best_fit_idx = len(self.Eopt)-1
                break
            # Check if there is scope to delete something else from these candidates 
            candidate, pods_to_remove = self.is_deletion_feasible(candidates_for_targetted_deletion, ms, rank_ms)
