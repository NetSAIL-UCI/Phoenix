...
from fair_allocation import balance_resources_v3
import copy
...
class PhoenixGreedy(PhoenixPlanner):
    def GlobalRankingModule(self, capacity):
        ...
        # capacity = round_to_single_digit(capacity)
        self.Resources = {i: nx.get_node_attributes(graph, "resources") for i, graph in self.graphs}
        ...
        self.Utilities = {}
        for key in self.Criticalities.keys():
            tags_dict = self.Criticalities[key]
            self.Utilities[key] = {key: 10**(10-tags_dict[key]) for key in tags_dict.keys()}
        WithinAppRank = {}
        for i, g in self.graphs:
            # the reason appranks are reversed is because we are taking the reverse in self.GlobalRank
            total_nodes = len(g.nodes)
            appranki = [ele for ele in self.AppRank if ele[0] == i]
            appRanks = {ele[1]: (total_nodes - i)  for i,ele in enumerate(appranki)}
            WithinAppRank[i] = appRanks
        ...
        def CustomKey(s):
            ...
            # self.Alloted[gid] += self.Resources[gid][nodeid]
            # res = self.Alloted[gid] - self.FS[gid]
            # return (self.Prices[gid][nodeid], WithinAppRank[gid][nodeid])
            reslog.append((self.Prices[gid][nodeid], WithinAppRank[gid][nodeid],))
            return (self.Prices[gid][nodeid], WithinAppRank[gid][nodeid])
        self.GlobalRank = sorted(self.AppRank, key=CustomKey, reverse=True)
        index = None
        remaining = copy.deepcopy(capacity)
        unscheduled = set()
        nodes_to_activate = []
...
# class PhoenixKnapsack(PhoenixPlanner):
#     def GlobalRankingModule(self, capacity):
#         from fair_allocation import water_filling
#         from knapsack import build_knapsack_input, KnapSackModule
#         capacity = round_to_single_digit(capacity)
